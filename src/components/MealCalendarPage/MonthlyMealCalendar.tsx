...
interface MonthlyMealCalendarProps {
  onDateClick?: (date: Date, meal: DayMeals) => void;
}

function MonthlyMealCalendar({ onDateClick }: MonthlyMealCalendarProps) {
  ...
  // { 'yyyy-MM-dd': { lunch: [{dishName, calorie}], dinner: [...] } }
  const [mealMap, setMealMap] = useState<Record<string, DayMeals>>({});
  ...
  useEffect(() => {
    ...
    async function fetchMeals() {
      ...
      try {
        const res = await axios.get<MonthMealResponse>(
        ...
        const byDate: Record<string, DayMeals> = {};
  ...
  const renderMealPreview = (day: Date) => {
    const key = toYMD(day);
            ...
            <div className="meal-text">
              {lunchList.flatMap((item, i) =>
                item.dishName.split(",").map((dish, idx) => (
                  <span key={`l-${i}-${idx}`} className="meal-line">
                    {dish.trim()}
                  </span>
                ))
              )}
          ...
          <div className="dinner">
            ...
            <div className="meal-text">
              {dinnerList.flatMap((item, i) =>
                item.dishName.split(",").map((dish, idx) => (
                  <span key={`d-${i}-${idx}`} className="meal-line">
                    {dish.trim()}
                  </span>
                ))
              )}
  ...
  const title = useMemo(() => {
    const m = parseInt(currentMonthKey.split("-")[1], 10);
